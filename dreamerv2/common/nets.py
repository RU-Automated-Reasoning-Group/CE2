...
class Encoder(common.Module):
    ...
    def __init__(
            self, shapes, state_key, cnn_keys=r'.*', mlp_keys=r'.*', act='elu', norm='none',
    ...
    def __call__(self, data):
        shape = self.shapes[self.state_key]
        batch_dims = data[self.state_key].shape[:-len(shape)]
        data = {
...
class L3P_GC_Distance(common.Module):
    def __init__(self, act='elu', layers=4, units=128, out_dim=1, input_type='feat', normalize_input=False):
    ...
    def __call__(self, gc_obs, no_softmax=True):
...
class DistLayer(common.Module):
    ...
    def __call__(self, inputs):
        ...
        out = tf.reshape(out, tf.concat(
            [tf.shape(inputs)[:-1], self._shape], 0))
...
class Cluster(nn.Module):
    ...
    def __init__(self, config, embed_size):
        ...
        self.n_mix = config.cluster["n_latent_landmarks"]

        self.z_dim = embed_size

        self.comp_mean = nn.Parameter(torch.randn(
            self.n_mix, self.z_dim) * np.sqrt(1.0 / self.n_mix))

        self.comp_logstd = nn.Parameter(torch.randn(
            1, self.z_dim) * 1 / np.e, requires_grad=True)

        self.mix_logit = nn.Parameter(torch.ones(
            self.n_mix), requires_grad=config.cluster["learned_prior"])
    ...
    def component_log_prob(self, x):
        ...
        assert x.ndim == 3 and x.size(
            1) == self.n_mix and x.size(2) == self.z_dim

        comp_logstd = torch.clamp(
            self.comp_logstd, self.config.cluster["LOG_STD_MIN"], self.config.cluster["LOG_STD_MAX"])

        comp_dist = Normal(self.comp_mean, torch.exp(comp_logstd))

        comp_log_prob = comp_dist.log_prob(x).sum(dim=-1)

        return comp_log_prob

    def sample(self, candidate_num, sample_num):

        comp_logstd = torch.clamp(
            self.comp_logstd, self.config.cluster["LOG_STD_MIN"], self.config.cluster["LOG_STD_MAX"])
        comp_dist = Normal(self.comp_mean, torch.exp(comp_logstd))

        points_sampled = comp_dist.sample_n(candidate_num)

        points_sampled = points_sampled.reshape(-1, self.z_dim)

        log_comp_probs = self.component_log_prob(points_sampled)

        log_mix_probs = torch.log_softmax(self.mix_logit, dim=-1).unsqueeze(0)
        log_prob_points_sampled = torch.logsumexp(
            log_mix_probs + log_comp_probs, dim=-1, keepdim=True)

        log_prob_points_sampled = log_prob_points_sampled.squeeze()

        min_prob_indices = torch.topk(
            log_prob_points_sampled, sample_num, largest=False).indices

        selected_points = points_sampled[min_prob_indices]

        return selected_points

    def cluster_to_x(self, x):
        ...
        assert log_mix_probs.size(
            0) == 1 and log_mix_probs.size(1) == self.n_mix

        prior_prob = torch.softmax(self.mix_logit, dim=0).unsqueeze(0)

        log_comp_probs = self.component_log_prob(x)

        cluter_max_prob = torch.argmax(log_comp_probs, dim=-1)

        return cluter_max_prob, log_comp_probs
    ...
    def forward(self, x, with_elbo=True):
        ...
        log_prob_x = torch.logsumexp(
            log_mix_probs + log_comp_probs, dim=-1, keepdim=True)

        log_posterior = log_comp_probs + log_mix_probs - log_prob_x

        posterior = torch.exp(log_posterior)

        if with_elbo:
            kl_from_prior = kl_divergence(Categorical(
                probs=posterior), Categorical(probs=prior_prob))
    ...
    def assign_centroids(self, x):
        ...
        self.mix_logit = nn.Parameter(torch.ones(
            self.n_mix), requires_grad=self.config.cluster["learned_prior"])
